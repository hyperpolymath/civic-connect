...
# civic-connect ABI/FFI Documentation
...
└─────────────────┬───────────────────────────┘
                  │
                  │ compiled to libcivic-connect.so/.a
                  ▼
┌─────────────────────────────────────────────┐
...
│       └── include/
│           └── civic-connect.h   # C header (optional, can be generated)
│
├── generated/                  # Auto-generated files
│   └── abi/
│       └── civic-connect.h       # Generated from Idris2 ABI
│
...
```bash
cd src/abi
idris2 --cg c-header Types.idr -o ../../generated/abi/civic-connect.h
```
...
```c
#include "civic-connect.h"

int main() {
    void* handle = civic-connect_init();
    if (!handle) return 1;

    int result = civic-connect_process(handle, 42);
    if (result != 0) {
        const char* err = civic-connect_last_error();
        fprintf(stderr, "Error: %s\n", err);
    }

    civic-connect_free(handle);
    return 0;
}
```
...
Compile with:
```bash
gcc -o example example.c -lcivic-connect -L./zig-out/lib
```

### From Idris2

```idris
import civic-connect.ABI.Foreign
...
```rust
#[link(name = "civic-connect")]
extern "C" {
    fn civic-connect_init() -> *mut std::ffi::c_void;
    fn civic-connect_free(handle: *mut std::ffi::c_void);
    fn civic-connect_process(handle: *mut std::ffi::c_void, input: u32) -> i32;
}

fn main() {
    unsafe {
        let handle = civic-connect_init();
        assert!(!handle.is_null());

        let result = civic-connect_process(handle, 42);
        assert_eq!(result, 0);
...
```julia
const libcivic-connect = "libcivic-connect"

function init()
    handle = ccall((:civic-connect_init, libcivic-connect), Ptr{Cvoid}, ())
    handle == C_NULL && error("Failed to initialize")
    handle
end

function process(handle, input)
    result = ccall((:civic-connect_process, libcivic-connect), Cint, (Ptr{Cvoid}, UInt32), handle, input)
    result
end

function cleanup(handle)
    ccall((:civic-connect_free, libcivic-connect), Cvoid, (Ptr{Cvoid},), handle)
end
...
2. **Generate C header**
   ```bash
   idris2 --cg c-header src/abi/Types.idr -o generated/abi/civic-connect.h
   ```
